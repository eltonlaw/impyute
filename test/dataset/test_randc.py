...
def test_raise_error_nlevel_exceed_shape():
    with pytest.raises(error.BadInputError) as e:
        randc(shape=(2, 2))
...
def test_nlevel_categories(nlevels, shape):
    """ideally the returned matrix should have nlevel+1 different categories,
    +1 because the Corrupt class introduce np.nan however, if the missing value
    introduced by Corrupt class happens to replace a group of categories, the
    unique category number would be < nlevel + 1
    """
